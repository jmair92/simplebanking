...
class Bank:
    ...
    def check_account(self, card_number, pin):
        self.cur.execute(f'SELECT * FROM card WHERE number = {card_number} AND pin = {pin}')
        bank_data = self.cur.fetchone()
        if bank_data:
            return True
        else:
            return False
    ...
    def add_income(self, card_number, amount):
        ...
        self.cur.execute(f'UPDATE card SET balance = {card[0][3] + amount} WHERE number = {card_number}')
        self.conn.commit()
